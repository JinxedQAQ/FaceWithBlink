...
class GenModel():
    def __init__(self, opt):
        ...
        self.blink_data = self.Tensor(opt.batchSize, opt.test_audio_video_length, opt.image_size) # 12x256
    ...
    def set_test_input(self, input):
        ...
        self.blink_data.resize_(input_blinkdata.size()).copy_(input_blinkdata)
    ...
    def test_train(self):
        ...
        self.test_mask[:, :, 80:144, 40:208] = 0.001
        self.real_A = Variable(self.input_A)
        self.blink_variable = Variable(self.blink_data)
        ...
        self.blink_variable = self.blink_variable.view(-1, self.opt.sequence_length, 3)
        self.audio_embeddings = torch.cat((self.blink_variable, self.audio_embeddings[:,:,3:]),2)
    ...
    def audio_sequence_generation(self):
        self.audio_embeddings = self.audio_embeddings.view(-1, self.opt.sequence_length, self.opt.feature_length)
        self.blink_variable = self.blink_variable.view(-1, self.opt.sequence_length, 3)
        self.audio_embeddings = torch.cat((self.audio_embeddings[:,:,:253], self.blink_variable),2)
        audio_gen_fakes = []
